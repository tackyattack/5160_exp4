...
uint8_t mount_drive(uint8_t xdata *xram_data_array)
{
  ...
  uint16_t temp16, RootEntCnt, RsvdSecCnt, FATSz16;
  uint32_t ClusterCount, FATSz, TotSec, DataSec, MBR_RelSec, FATSz32, RootClus;
  uint8_t *data_array;
  ...
  if(FATSz32 != 0)
  {
    TotSec = read16(BPB_TotSec16_offset, data_array);
  }
  else
  {
  ...
  ClusterCount = DataSec / drive_values->BytesPerSec;
  ...
  else if(ClusterCount < 65525)
  {
    ...
    drive_values->FirstRootDirSec = RsvdSecCnt + (NumFAT * FATSz16);
  }
  else
  {
    // FAT32
